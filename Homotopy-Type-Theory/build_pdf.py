#!/usr/bin/env python3
"""
build_pdf.py — Build the Homotopy Type Theory textbook as a PDF.
...
DEFAULT_OUT = BOOK_ROOT / "hott-textbook-built.pdf"

YAML_HEADER = """\
---
title: "Homotopy Type Theory"
subtitle: "From Dependent Types to Synthetic Homotopy Theory"
documentclass: book
...
\definecolor{chapcolor}{RGB}{30,50,100}
...
def chapter_dirs() -> list:
    ...
    if not chapters.exists():
        return []
    return sorted(
        (d for d in chapters.iterdir() if d.is_dir()),
        key=lambda d: _leading_int(d.name),
    )

def section_files(chap: Path) -> list:
    return sorted(
        (f for f in chap.rglob("*.md")),
        key=lambda f: _leading_int(f.name),
    )

def flat_chapter_files() -> list:
    chapters = BOOK_ROOT / "chapters"
    if not chapters.exists():
        return []
    return sorted(
        (f for f in chapters.glob("*.md")),
        key=lambda f: _leading_int(f.name),
    )
...
def assemble(draft: bool = False) -> str:
    ...
    if cdirs:
        ...
        for cdir in cdirs:
            ...
            sfiles = [f for f in section_files(cdir) if f.name != "README.md"]
        ...
        # Flat chapter files
        ffiles = flat_chapter_files()
        if draft: ffiles = ffiles[:4]
        for f in ffiles:
            body = strip_front_matter(read_file(f))
            body = clean_first_heading(body)
            parts.append(raw_latex(r"\cleardoublepage"))
            parts.append(body + "\n\n")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Build HoTT textbook PDF.")
    ...
    print("Building Homotopy Type Theory textbook PDF\n" + "=" * 42)
