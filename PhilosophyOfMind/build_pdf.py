#!/usr/bin/env python3
"""
build_pdf.py — Build "Philosophy of Mind" as a PDF.
...
DEFAULT_OUT = BOOK_ROOT / "philosophy-of-mind.pdf"

YAML_HEADER = """\
---
title: "Philosophy of Mind"
subtitle: "Consciousness, Intentionality, and the Nature of Mental States"
documentclass: book
...
\definecolor{chapcolor}{RGB}{60,20,80}
...
def clean_first_heading(text: str) -> str:
    prefix_re = re.compile(r"^(?:\d+_[A-Za-z_]+\s*[:\-]?\s*|Chapter\s+\d+[:\s]+|Section\s+[\d.]+[:\s]+)")
...
def chapter_dirs() -> list:
    return sorted(
        (d for d in BOOK_ROOT.iterdir() if d.is_dir() and re.match(r"\d+_", d.name)),
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Build Philosophy of Mind PDF.")
    ...
    print("Building Philosophy of Mind textbook PDF\n" + "=" * 40)
