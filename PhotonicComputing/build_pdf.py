#!/usr/bin/env python3
"""
build_pdf.py — Build "Photonic Computing: Light-Speed Information Processing" as a PDF.
...
DEFAULT_OUT = BOOK_ROOT / "photonic-computing.pdf"

YAML_HEADER = """\
---
title: "Photonic Computing: Light-Speed Information Processing"
subtitle: "From Integrated Photonics to Optical Neural Networks"
documentclass: book
...
\definecolor{chapcolor}{RGB}{0,80,120}
...
def clean_first_heading(text: str) -> str:
    prefix_re = re.compile(r"^(?:Chapter\s+\d+[:\s]+|Section\s+[\d.]+[:\s]+)")
...
def chapter_dirs() -> list:
    ...
    if not book_dir.exists():
        return []
    return sorted(
        (d for d in book_dir.iterdir() if d.is_dir()),
        key=lambda d: _leading_int(d.name),
    )

def outline_file() -> Path:
    return BOOK_ROOT / "photonic_computing_book_outline.md"
...
def assemble(draft: bool = False) -> str:
    ...
    if cdirs:
        ...
        for cdir in cdirs:
            ...
            body = strip_front_matter(read_file(readme)) if readme.exists() else f"# {cdir.name.replace('-', ' ').title()}\n"
        ...
        # Fallback: use the outline file as the sole chapter
        outline = outline_file()
        if outline.exists():
            body = strip_front_matter(read_file(outline))
            parts.append(body + "\n\n")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Build Photonic Computing PDF.")
    ...
    print("Building Photonic Computing textbook PDF\n" + "=" * 40)
