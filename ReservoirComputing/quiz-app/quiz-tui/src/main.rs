...
struct App {
    ...
    selected: usize,
    /// Accumulated text for FreeText questions.
    input: String,
    should_quit: bool,
}
            ...
            match key.code {
                KeyCode::Enter => {
                    if !self.input.is_empty() {
                        ...
                        let q = self.session.quiz.questions[q_idx].clone();
...
fn draw_question(frame: &mut Frame, app: &App) {
    ...
    let gauge = Gauge::default()
        .block(
            Block::default()
                .borders(Borders::ALL)
                .title(format!(" {} ", app.session.quiz.title)),
        )
...
fn draw_feedback(frame: &mut Frame, was_correct: bool, question: &Question) {
    ...
    if let Some(exp) = question.explanation() {
        lines.push(Line::from(Span::styled(
            exp,
