...
![A simulated day of response times, log scale. The body of the distribution sets the mean and the median; the second mode — a few percent of slow responses — is invisible in both and is where every complaint comes from. The figure's percentiles differ from the table's because every day differs; the shape is what recurs.](../../figures/latency_percentiles.svg){width=90%}
