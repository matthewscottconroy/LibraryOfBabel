#!/usr/bin/env python3
"""
build_pdf.py — Build "Formal Logic and Theorem Proving" as a PDF.
...
DEFAULT_OUT = BOOK_ROOT / "theorem-proving.pdf"

YAML_HEADER = """\
---
title: "Formal Logic and Theorem Proving"
subtitle: "From Propositional Logic to Categorical Foundations"
documentclass: book
...
\definecolor{chapcolor}{RGB}{10,60,40}
...
def clean_first_heading(text: str) -> str:
    prefix_re = re.compile(r"^(?:Chapter\s+\d+[:\s]+|Section\s+[\d.]+[:\s]+|ch\d+_[a-z_]+\s*)")
...
def chapter_dirs() -> list:
    ...
    return sorted(
        (d for d in textbook.iterdir() if d.is_dir()),
        key=lambda d: _leading_int(d.name),
    )

def section_files(chap: Path) -> list:
    return sorted(
        (f for f in chap.rglob("*.md") if f.name != "README.md"),
        key=lambda f: _leading_int(f.name),
    )

def problem_files() -> list:
    problems_dir = BOOK_ROOT / "problems"
    if not problems_dir.exists():
        return []
    return sorted(problems_dir.rglob("*.md"), key=lambda f: _leading_int(f.parent.name))

def assemble(draft: bool = False) -> str:
    parts = [YAML_HEADER]

    cdirs = chapter_dirs()
    if draft: cdirs = cdirs[:4]

    for cdir in cdirs:
        readme = cdir / "README.md"
        body = strip_front_matter(read_file(readme)) if readme.exists() else f"# {cdir.name.replace('_', ' ')}\n"
    ...
    if not draft:
        pfiles = problem_files()
        if pfiles:
            parts.append(raw_latex(r"\appendix" + "\n" + r"\renewcommand{\chaptername}{Problems}"))
            parts.append("# Problem Sets {.unnumbered}\n\n")
            for pf in pfiles:
                pbody = strip_front_matter(read_file(pf))
                parts.append(pbody + "\n\n")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Build Formal Logic and Theorem Proving PDF.")
    ...
    print("Building Theorem Proving textbook PDF\n" + "=" * 38)
