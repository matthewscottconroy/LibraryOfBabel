...
/// Fallback model id when none is configured via `subject.toml` or env.
pub const DEFAULT_MODEL: &str = "claude-sonnet-5";
const ANTHROPIC_VERSION: &str = "2023-06-01";

/// Resolve the model id to use for generation.
///
/// Priority: explicit `subject_model` (the `model` key in `subject.toml`) →
/// `ANTHROPIC_MODEL` env var → [`DEFAULT_MODEL`].
pub fn resolve_model(subject_model: Option<&str>) -> String {
    subject_model
        .map(|s| s.trim())
        .filter(|s| !s.is_empty())
        .map(|s| s.to_string())
...
impl ClaudeGenerator {
    ...
    /// - `model`: pass the `subject.toml` `model` key (or `None`); the value
    ///   is resolved via [`resolve_model`] (env `ANTHROPIC_MODEL`, then
    ///   [`DEFAULT_MODEL`]).
    pub fn new(
        api_key: Option<String>,
    ...
    /// The resolved model id used for generation.
