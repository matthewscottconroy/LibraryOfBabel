...
mod tests {
    ...
    fn unique_dir(tag: &str) -> PathBuf {
        let nanos = std::time::SystemTime::now()
            ...
            .unwrap()
            .as_nanos();
        std::env::temp_dir().join(format!("quiz_storage_{tag}_{nanos}"))
    }

    #[test]
    fn save_load_roundtrip() {
        let dir = unique_dir("roundtrip");
        let mut p = UserProfile::new("Ada Lovelace");
        p.record_answer(2, true, "q-1", 3);
        p.record_answer(2, false, "q-2", 1);

        save_profile(&dir, &p).expect("save");
        let loaded = load_profile(&dir, "Ada Lovelace").expect("load");

        assert_eq!(loaded.name, "Ada Lovelace");
        assert_eq!(loaded.total_answered(), 2);
        assert_eq!(loaded.mastery[&2].total_seen, 2);
        assert_eq!(loaded.mastery[&2].total_correct, 1);
    ...
    fn load_or_create_creates_then_loads() {
        let dir = unique_dir("loadcreate");
        let created = load_or_create(&dir, "Grace").expect("create");
        assert_eq!(created.name, "Grace");
        // Second call must load the same profile, not error.
        let again = load_or_create(&dir, "Grace").expect("load");
        assert_eq!(again.user_id, created.user_id);
        let _ = std::fs::remove_dir_all(&dir);
    }

    #[test]
    fn corrupt_profile_is_backed_up_and_returns_none() {
        let dir = unique_dir("corrupt");
        std::fs::create_dir_all(&dir).unwrap();
        // safe_name("Corrupt User") -> "corrupt_user"
        let path = dir.join("corrupt_user.json");
        std::fs::write(&path, b"{ this is not valid json").unwrap();

        let loaded = load_profile(&dir, "Corrupt User");
        assert!(loaded.is_none(), "corrupt profile must not deserialize");

        // Original is renamed to a .backup.json for inspection.
        let backup = path.with_extension("backup.json");
