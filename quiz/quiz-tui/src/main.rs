...
use quiz_core::{
    load_question_bank, load_quiz, question::Question, Quiz, Session, SubjectConfig,
};

// ── Source resolution ─────────────────────────────────────────────────────────

/// Resolve what to quiz on.
///
/// Priority:
///   1. `--subject <dir>` (or `-s <dir>`)  → load `<dir>/questions` bank
///   2. a positional file path             → load that single quiz file
///   3. `$QUIZ_SUBJECT` / current directory → load the resolved subject's bank
fn resolve_quiz() -> Result<Quiz, Box<dyn std::error::Error>> {
    ...
    while i < argv.len() {
        match argv[i].as_str() {
            "--subject" | "-s" => {
                i += 1;
                subject = argv.get(i).map(PathBuf::from);
            }
            other if other.starts_with("--subject=") => {
                subject = Some(PathBuf::from(&other["--subject=".len()..]));
            }
    ...
    // Single-file mode only when a file is given and no explicit subject.
    if subject.is_none() {
        if let Some(path) = positional {
            return load_quiz(PathBuf::from(path));
        }
    }

    let config = SubjectConfig::resolve(subject)?;
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    let quiz = resolve_quiz()?;
    if quiz.questions.is_empty() {
        eprintln!("No questions found for this subject.");
        eprintln!("Pass --subject <dir> (containing subject.toml + questions/), or a quiz file path.");
        std::process::exit(1);
    }
